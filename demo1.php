...
// $db = new MysqliDb ('localhost', 'root', '', 'test_db');
...
if($id) echo 'user was created. Id=' . $id;
...
if(!$ids)  echo 'insert failed: ' . $db->getLastError();
else echo 'new users inserted with following id\'s: ' . implode(', ', $ids);
...
// );
// $db->where ('id', 1);
// if ($db->update ('users', $data)) echo $db->count . ' records were updated';
...
// if ($db->getLastErrno() === 0) echo 'Update succesfull';
// else echo 'Update failed. Error: '. $db->getLastError();
